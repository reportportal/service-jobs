...
public class NotifyUserExpirationJob extends BaseJob {
  ...
  private final String SELECT_USERS_FOR_NOTIFY = "WITH user_last_action AS ( "
      ...
      + "  u.id as user_id, "
      + "  u.email as email, "
      + "  (((EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT / 86400000) - GREATEST((u.metadata->'metadata'->>'last_login')::BIGINT / 86400000, COALESCE(MAX(EXTRACT(EPOCH FROM ak.last_used_at) * 1000)::BIGINT / 86400000, 0))) AS inactivityPeriod "
      + "FROM "
      + "  users u "
