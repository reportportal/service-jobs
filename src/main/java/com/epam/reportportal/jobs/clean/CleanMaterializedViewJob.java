...
public class CleanMaterializedViewJob extends BaseJob {
  ...
  public CleanMaterializedViewJob(JdbcTemplate jdbcTemplate,
      @Value("${rp.environment.variable.clean.view.batch}") Integer batchSize,
  ...
  private List<StaleMaterializedView> getStaleViews(LocalDateTime timeBound) {
    final Map<String, Object> selectParams = Map.of(TIME_BOUND_PARAM, timeBound, BATCH_SIZE_PARAM,
        batchSize);
    return namedParameterJdbcTemplate.query(SELECT_STALE_VIEWS, selectParams,
        new BeanPropertyRowMapper<>(StaleMaterializedView.class));
  }

  private int removeExistingViews(List<String> viewNames) {
    final List<String> existingViews = namedParameterJdbcTemplate.queryForList(
        SELECT_EXISTING_VIEWS_BY_NAME_IN,
        Map.of(NAMES_PARAM, viewNames),
        String.class
    );

    existingViews.forEach(
        name -> namedParameterJdbcTemplate.update(String.format(DROP_MATERIALIZED_VIEW, name),
            Map.of()));
    return existingViews.size();
  }

  private int removeStaleViews(List<String> viewNames) {
    return namedParameterJdbcTemplate.update(DELETE_STALE_VIEWS_BY_NAMES,
        Map.of(NAMES_PARAM, viewNames));
  }
}
